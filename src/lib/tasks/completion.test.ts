...
describe("getCompletionPatch", () => {
  ...
  test("reabre no status anterior e limpa a conclusão", () => {
    const patch = getCompletionPatch({ ...task, status: "done", previousStatus: "waiting" });
    expect(patch).toEqual({ status: "waiting", completedAt: "", completedBy: "" });
  });
  ...
  test("impede desfazer sobre uma alteração posterior", () => {
    expect(() => assertExpectedTaskStatus({ ...task, status: "waiting" }, "done")).toThrow();
