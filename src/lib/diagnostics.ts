...
const mapQuestionFromDb = (question: DbQuestionRow, rules: DbRuleRow[]): TemplateQuestion => {
  ...
  return {
    ...
    required: question.required ?? metadata.required ?? true,
    helperText: question.helper_text || metadata.helperText || "",
    placeholder: metadata.placeholder || "",
    order: question.position ?? 0,
    minValue: question.min_value ?? metadata.minValue ?? null,
    maxValue: question.max_value ?? metadata.maxValue ?? null,
