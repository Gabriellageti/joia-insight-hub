    ...
    <div className="rounded-lg border bg-card p-3">
      <Collapsible className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <CollapsibleTrigger asChild><Button variant="outline"><Filter className="mr-2 h-4 w-4" />Filtros{hasFilters ? " ativos" : ""}</Button></CollapsibleTrigger>
          {hasFilters && <Button variant="ghost" size="sm" onClick={onClear}><X className="mr-1 h-4 w-4" />Limpar</Button>}
        </div>
          <CollapsibleContent className="mt-3 rounded-md border bg-muted/20 p-3">
            <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
              ...
              <div className="flex items-center gap-2 px-1"><Checkbox id="overdue-filter" checked={values.overdue} onCheckedChange={(checked) => setValue("overdue", checked === true)} /><Label htmlFor="overdue-filter">Somente atrasadas</Label></div>
            </div>
          </CollapsibleContent>
      </Collapsible>
    </div>
