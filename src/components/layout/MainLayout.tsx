...
export function MainLayout({ children }: MainLayoutProps) {
  ...
  return (
    <SidebarProvider open={sidebarOpen} onOpenChange={handleSidebarChange}>
      <div className="min-h-screen flex w-full min-w-0 overflow-x-clip">
        <AppSidebar />
        <div className="min-w-0 flex-1 flex flex-col">
          <TopBar />
          <main className="min-w-0 flex-1 overflow-y-auto overflow-x-clip p-3 sm:p-4 lg:p-6 bg-background">
            {children}
          </main>
        </div>
      </div>
