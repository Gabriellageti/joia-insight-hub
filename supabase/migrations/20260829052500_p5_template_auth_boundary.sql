...
BEGIN
  ...
  SELECT p_project_id,assignee,'editor',(SELECT auth.uid()) FROM (SELECT p_fallback_assignee assignee UNION SELECT default_assignee_id FROM public.task_templates WHERE project_template_id=p_template_id) candidates
  WHERE assignee IS NOT NULL AND private.can_assign_workspace_task(assignee,v_project.workspace_id) ON CONFLICT(project_id,user_id) DO NOTHING;
  INSERT INTO public.project_stages(workspace_id,project_id,source_template_stage_id,title,description,position)
  SELECT v_project.workspace_id,p_project_id,s.id,s.title,s.description,s.position FROM public.project_template_stages s WHERE s.template_id=p_template_id ON CONFLICT(project_id,position) DO NOTHING;
